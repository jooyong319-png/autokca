...
export function Ballot({
  ...
  const sayPlaceholder = (() => {
    /* 🔴 결과가 아직 안 나온 구간(5표 미만)에서는 **비율을 근거로 말하지 않는다.**
       `myPct`는 임계치 아래에서 의미가 없다 — 2표에서 "소수 의견이십니다"는
       근거가 없고, 그건 숫자를 지어내는 것과 같은 종류의 거짓이다. */
    if (!choice || !showsResult(stage)) return '왜 그런지 한 줄로';
  ...
  return (
    ...
    >
      ...
      <div className={styles.choices}>
         ...
         *
         *  기표칸 위에 **크게** 찍는다 — 실제로 처리한 서류에 도장을 찍는 자리가 여기다.
         *  카드 구석의 작은 배지였을 때는 눈에 안 들어왔다.
         *
         *  ⚠️ `pointer-events: none`이 **필수**다. 이 위를 덮으면 기표 번복을 못 한다
         *     (다른 쪽을 눌러 바꿀 수 있어야 한다).
      ...
      {/* 🔴 투표하면 **바로** 띄운다(7차 §3).
       *
       *  전에는 `hasResult`(5표 이상 + 투표함)를 조건에 넣었다 — 7차 §1의
       *  "결과가 안 보이는 상태에서 입력창을 띄우면 무엇에 반응해서 쓰라는 건지
       *  알 수 없다"를 그대로 따른 것이다. **콜드스타트에서 그 전제가 틀렸다.**
       *
       *  실측: 103개 중 5표를 넘긴 질문이 5개뿐이라 **98개에서는 투표해도 입력창이
       *  안 떴다.** 만들어 놓고 거의 안 보이는 기능이었다.
       *
       *  게다가 색인 조건이 "20표 **그리고** 댓글 1개"라 댓글을 못 받으면 색인이
       *  영영 안 열린다 — 표가 적을 때야말로 댓글이 더 필요하다.
       *
       *  방금 투표한 사람은 **무엇에 반응하는지 안다**(질문과 자기 선택).
       *  결과는 있으면 좋은 것이지 전제가 아니다.
       *  개표 중일 때는 `sayPlaceholder`가 중립 문구로 바뀐다.
       *
       *  ⚠️ 상세 페이지의 **주** 카드에서는 렌더하지 않는다 — 그 페이지에는 아래에
       *  진짜 댓글 섹션(`Comments`)의 입력 폼이 이미 있어서 입력창이 둘이 된다.
       *  이어지는 피드의 카드들은 각자 다른 안건이므로 그대로 보여준다.
       *
       *  이미 썼으면 감춘다(1안건 1의견). */}
      {!standalone && choice && !locked && (
        <CardSay
