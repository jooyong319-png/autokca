...
- **표본 임계치는 `lib/tiers.ts` 한 곳에만 있다.** 화면·제목·설명·OG·구조화 데이터가
      전부 그 파일을 거치므로 값을 여기저기 복사하지 않는다.

      | 게이트 | 값 | 무엇을 가리나 |
      |---|---|---|
      | `TIER.RESULT` | **5표** | 퍼센트·막대 (UX) |
      | `TIER.VERDICT` | **20표** | 판정 카피 "이상한 사람입니다" (UX) |
      | `TIER.INDEX` | **20표 + 댓글 1개** | `noindex` 해제 (SEO) |

      UX와 SEO 게이트는 **목적이 다르므로 값이 다르다**(6차 §1-1). 사용자는 결과를 보는데
      크롤러는 아직 못 보는 상태가 정상이다.
      **참여자 수는 임계치와 무관하게 항상 노출한다** — 감출 것은 퍼센트지 참여 규모가 아니다.

      > ⚠️ 여기 "참여자 수는 100표 미달이면 감춘다(`TURNOUT_THRESHOLD`)"라고 적혀 있었는데
      > **그 상수는 1차 개선에서 이미 삭제됐다.** `lib/tiers.ts`와 값이 겹쳐서였다.
      > 문서만 남아 6차 문서가 그걸 근거로 잘못된 진단을 했다 — 문서를 고치지 않으면
      > 다음 사람이 같은 함정에 빠진다.
